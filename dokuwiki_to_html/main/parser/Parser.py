...
class Parser:
    ...
    def parse(self):
        try:
            ...
            raise ParseError("Syntax error at line " + str(self.current_line) + ". Execution cancelled.")
    ...
    def do_parse(self):
        ...
        while self.next_token_index < len(self.token_list):
            if self.current_token.is_heading():
                ...
                self.read_next_token()
                self.current_line += 2
            elif self.current_token.token == TokenType.new_line:
                self.current_line += 1
            try:
                self.read_next_token()
            except IndexError:
    ...
    def parse_image(self):
        ...
        if link_parts[2] != "":
            ...
            width = ''.join(filter(lambda n: n.isdigit(), width))
            height = ''.join(filter(lambda n: n.isdigit(), height))
            if height != "":
